...
mod tests {
    ...
    fn sha_match_is_exact() {
        assert_eq!(score_confidence(true, false, false), Confidence::Exact);
    }

    #[test]
    fn sha_match_overrides_timestamp() {
        assert_eq!(score_confidence(true, true, true), Confidence::Exact);
    }

    #[test]
    fn timestamp_with_mapping_is_high() {
        assert_eq!(score_confidence(false, true, true), Confidence::High);
    }

    #[test]
    fn timestamp_only_is_low() {
        assert_eq!(score_confidence(false, true, false), Confidence::Low);
    }

    #[test]
    fn nothing_is_unmatched() {
        assert_eq!(score_confidence(false, false, false), Confidence::Unmatched);
    }
}
